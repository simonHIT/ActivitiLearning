...
<script type="text/javascript">
    ...
    layui.use(['form', 'layer', 'table', 'laytpl'], function () {
        ...
        tableIns = table.render({
            elem: '#providerList',
            url: '${simonBasePath}/provider/loadProviders.action',
            cellMinWidth: 95,
            page: true,
            height: "full-200",
            limits: [10, 15, 20, 25],
        ...
        $(".search_btn").on("click", function () {
            ...
            table.reload('providerListTable', {
                url: '${simonBasePath}/provider/loadProviders.action?' + params
            });
        });
        //监听头工具栏事件
        ...
        function toAddProvider() {
            var index = layui.layer.open({
                ...
                type: 2,//ifream层
                area: ["800px", "500px"],
                content: "${simonBasePath}/provider/toAddProvider.action",
        ...
        function updateProvider(data) {
            var index = layui.layer.open({
                ...
                type: 2,
                area: ["800px", "500px"],
                content: "${simonBasePath}/provider/toUpdateProvider.action?id=" + data.id,
                success: function (layero, index) {
                    setTimeout(function () {
                        layui.layer.tips('点击此处返回供应商列表', '.layui-layer-setwin .layui-layer-close', {
                            tips: 3
        ...
        table.on('tool(providerList)', function (obj) {
            ...
            if (layEvent === 'edit') { //编辑
                ...
                layer.confirm('确定删【' + data.providername + '】供应商吗？', {icon: 3, title: '提示信息'}, function (index) {
                    $.post("${simonBasePath}/provider/deleteProvider.action", {
        ...
        function batchDelete() {
            ...
            if (data.length > 0) {
                ...
                layer.confirm('确定删除选中的供应商？', {icon: 3, title: '提示信息'}, function (index) {
                    $.post("${simonBasePath}/provider/batchDeleteProvider.action?" + ids, function (data) {
                        //刷新table
                        tableIns.reload();
                        //关闭提示框
                        layer.close(index);
