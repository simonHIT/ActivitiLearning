...
<script type="text/javascript">
    ...
    layui.use(['form', 'layer', 'table', 'laytpl'], function () {
        ...
        tableIns = table.render({
            elem: '#customerList',
            url: '${simonBasePath }/customer/loadCustomers.action',
            cellMinWidth: 95,
            page: true,
            height: "full-200",
            limits: [10, 15, 20, 25],
        ...
        $(".search_btn").on("click", function () {
            ...
            table.reload('customerListTable', {
                url: '${simonBasePath }/customer/loadCustomers.action?' + params
            });
        });
        ...
        function toAddCustomer() {
            var index = layui.layer.open({
                ...
                type: 2,//ifream层
                area: ["800px", "520px"],
                content: "${simonBasePath }/customer/toAddCustomer.action",
        ...
        function updateCustomer(data) {
            var index = layui.layer.open({
                ...
                type: 2,
                area: ["800px", "520px"],
                content: "${simonBasePath }/customer/toUpdateCustomer.action?id=" + data.id,
                success: function (layero, index) {
                    setTimeout(function () {
                        layui.layer.tips('点击此处返回客户列表', '.layui-layer-setwin .layui-layer-close', {
                            tips: 3
        ...
        table.on('tool(customerList)', function (obj) {
            ...
            if (layEvent === 'edit') { //编辑
                ...
                layer.confirm('确定删【' + data.customername + '】客户吗？', {icon: 3, title: '提示信息'}, function (index) {
                    $.post("${simonBasePath }/customer/deleteCustomer.action", {
        ...
        function batchDelete() {
            ...
            if (data.length > 0) {
                ...
                layer.confirm('确定删除选中的客户？', {icon: 3, title: '提示信息'}, function (index) {
                    $.post("${simonBasePath }/customer/batchDeleteCustomer.action?" + ids, function (data) {
                        //刷新table
                        tableIns.reload();
                        //关闭提示框
                        layer.close(index);
