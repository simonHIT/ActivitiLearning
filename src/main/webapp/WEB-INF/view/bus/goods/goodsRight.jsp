...
<script type="text/javascript">
    ...
    layui.use(['form', 'layer', 'table', 'laytpl'], function () {
        ...
        tableIns = table.render({
            elem: '#goodsList',
            url: '${simonBasePath }/goods/loadGoods.action',
            cellMinWidth: 95,
            page: true,
            height: "full-200",
            limits: [10, 15, 20, 25],
        ...
        $(".search_btn").on("click", function () {
            ...
            table.reload('goodsListTable', {
                url: '${simonBasePath }/goods/loadGoods.action?' + params
            });
        });
        //监听头工具栏事件
        ...
        function toAddGoods() {
            var index = layui.layer.open({
                ...
                type: 2,//ifream层
                area: ["700px", "500px"],
                content: "${simonBasePath }/goods/toAddGoods.action",
        ...
        function updateGoods(data) {
            var index = layui.layer.open({
                ...
                type: 2,
                area: ["700px", "500px"],
                content: "${simonBasePath }/goods/toUpdateGoods.action?id=" + data.id,
                success: function (layero, index) {
                    setTimeout(function () {
                        layui.layer.tips('点击此处返回商品列表', '.layui-layer-setwin .layui-layer-close', {
                            tips: 3
        ...
        table.on('tool(goodsList)', function (obj) {
            ...
            if (layEvent === 'edit') { //编辑
                ...
                layer.confirm('确定删【' + data.goodsname + '】商品吗？', {icon: 3, title: '提示信息'}, function (index) {
                    $.post("${simonBasePath }/goods/deleteGoods.action", {
        ...
        function batchDelete() {
            ...
            if (data.length > 0) {
                ...
                layer.confirm('确定删除选中的商品？', {icon: 3, title: '提示信息'}, function (index) {
                    $.post("${simonBasePath }/goods/batchDeleteGoods.action?" + ids, function (data) {
                        //刷新table
                        tableIns.reload();
                        //关闭提示框
                        layer.close(index);
