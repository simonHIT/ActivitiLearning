...
<script type="text/javascript">
    ...
    layui.use(['form', 'layer', 'table', 'laydate'], function () {
        ...
        tableIns = table.render({
            elem: '#noticeList',
            url: '${simonBasePath }/notice/loadAllNotices.action',
            cellMinWidth: 95,
        ...
        $(".search_btn").on("click", function () {
            ...
            table.reload('noticeListTable', {
                url: '${simonBasePath }/notice/loadAllNotices.action?' + params
            });
        });
        //监听头工具栏事件
        ...
        function toAddNotice() {
            var index = layui.layer.open({
                ...
                type: 2,//ifream层
                area: ["800px", "500px"],
                content: "${simonBasePath }/notice/toAddNotice.action",
        ...
        function updateNotice(data) {
            var index = layui.layer.open({
                ...
                type: 2,
                area: ["800px", "500px"],
                content: "${simonBasePath }/notice/toUpdateNotice.action?id=" + data.id,
                success: function (layero, index) {
                    setTimeout(function () {
                        layui.layer.tips('点击此处返回公告列表', '.layui-layer-setwin .layui-layer-close', {
                            tips: 3
        ...
        table.on('tool(noticeList)', function (obj) {
            ...
            if (layEvent === 'edit') { //编辑
                ...
                layer.confirm('确定删【' + data.title + '】公告吗？', {icon: 3, title: '提示信息'}, function (index) {
                    $.post("${simonBasePath}/notice/deleteNotice.action", {
        ...
        function batchDelete() {
            ...
            if (data.length > 0) {
                ...
                layer.confirm('确定删除选中的公告？', {icon: 3, title: '提示信息'}, function (index) {
                    $.post("${simonBasePath}/notice/batchDeleteNotice.action?" + ids, function (data) {
                        //刷新table
                        tableIns.reload();
                        //关闭提示框
                        layer.close(index);
        ...
        function show(id) {
            var index = layui.layer.open({
                ...
                type: 2,
                area: ["800px", "500px"],
                content: "${simonBasePath }/notice/showNotice.action?id=" + id,
                success: function (layero, index) {
                    setTimeout(function () {
                        layui.layer.tips('点击此处返回公告列表', '.layui-layer-setwin .layui-layer-close', {
                            tips: 3
