...
<script type="text/javascript">
    ...
    layui.use(['form', 'layer', 'laydate', 'table', 'laytpl'], function () {
        ...
        tableIns = table.render({
            elem: '#permissionList',
            url: '${simonBasePath}/permission/loadAllPermission.action',
            cellMinWidth: 95,
            page: true,
            height: "full-160",
        ...
        $(".search_btn").on("click", function () {
            ...
            //搜索
            tableIns.reload({
                url: '${simonBasePath}/permission/loadAllPermission.action?' + params
            })
        });
        ...
        function addPermission() {
            var index = layui.layer.open({
                ...
                type: 2,
                area: ["700px", '450px'],
                content: "${simonBasePath}/permission/toAddPermission.action",
        ...
        function batchDel() {
            ...
            if (data.length > 0) {
                ...
                layer.confirm('确定删除选中的权限？', {icon: 3, title: '提示信息'}, function (index) {
                    $.get("${simonBasePath}/permission/deletePermissionBatch.action?" + permissionIds, function (data) {
                        tableIns.reload();
                        layer.close(index);
                    })
                })
        ...
        table.on('tool(permissionList)', function (obj) {
            ...
            if (layEvent === 'edit') { //编辑
                ...
                layer.confirm('确定删除此权限？', {icon: 3, title: '提示信息'}, function (index) {
                    $.post("${simonBasePath}/permission/deletePermission.action", {
                        id: data.id
                    }, function (data) {
                        tableIns.reload();
                        layer.close(index);
        ...
        function updatePermission(data) {
            var index = layui.layer.open({
                ...
                type: 2,
                area: ["700px", '450px'],
                content: "${simonBasePath}/permission/toUpdatePermission.action?id=" + data.id,
                success: function (layero, index) {
                    setTimeout(function () {
                        layui.layer.tips('点击此处返回权限列表', '.layui-layer-setwin .layui-layer-close', {
                            tips: 3
