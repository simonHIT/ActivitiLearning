...
<script type="text/javascript">
    layui.use(['form', 'layer', 'laydate', 'table', 'laytpl'], function () {
        ...
        var tableIns = table.render({
            elem: '#logInfoList',
            url: '${simonBasePath }/logInfo/loadAllLogInfo.action',
            cellMinWidth: 95,
            page: true,
            height: "full-225",
            limit: 20,
        ...
        $(".search_btn").on("click", function () {
            ...
            //搜索
            tableIns.reload({
                url: '${simonBasePath }/logInfo/loadAllLogInfo.action?' + params
            })
        });
        ...
        function batchDel() {
            ...
            if (data.length > 0) {
                ...
                layer.confirm('确定删除选中的日志？', {icon: 3, title: '提示信息'}, function (index) {
                    $.post("${simonBasePath}/logInfo/deleteBatchLogInfo.action?" + logInfoId, function (data) {
                        layer.msg(data.msg);
                        tableIns.reload();
                        layer.close(index);
                    })
        ...
        table.on('tool(logInfoList)', function (obj) {
            ...
            if (layEvent === 'del') { //删除
                layer.confirm('确定删除此日志？', {icon: 3, title: '提示信息'}, function (index) {
                    $.post("${simonBasePath}/logInfo/deleteLogInfo.action", {
