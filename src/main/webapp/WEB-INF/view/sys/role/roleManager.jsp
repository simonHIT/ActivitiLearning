...
<script type="text/javascript">
    ...
    layui.use(['form', 'layer', 'table', 'laydate', 'laytpl'], function () {
        ...
        tableIns = table.render({
            elem: '#roleList',
            url: '${simonBasePath }/role/loadAllRoles.action',
            cellMinWidth: 95,
        ...
        $(".search_btn").on("click", function () {
            ...
            table.reload('roleListTable', {
                url: '${simonBasePath }/role/loadAllRoles.action?' + params
            });
        });
        //监听头工具栏事件
        ...
        function toAddRole() {
            var index = layui.layer.open({
                ...
                type: 2,//ifream层
                area: ["800px", "500px"],
                content: "${simonBasePath }/role/toAddRole.action",
        ...
        function updateRole(data) {
            var index = layui.layer.open({
                ...
                type: 2,
                area: ["800px", "500px"],
                content: "${simonBasePath }/role/toUpdateRole.action?id=" + data.id,
                success: function (layero, index) {
                    setTimeout(function () {
                        layui.layer.tips('点击此处返回角色列表', '.layui-layer-setwin .layui-layer-close', {
                            tips: 3
        ...
        table.on('tool(roleList)', function (obj) {
            ...
            if (layEvent === 'edit') { //编辑
                ...
                layer.confirm('确定删【' + data.name + '】角色吗？', {icon: 3, title: '提示信息'}, function (index) {
                    $.post("${simonBasePath}/role/deleteRole.action", {
        ...
        function batchDelete() {
            ...
            if (data.length > 0) {
                ...
                layer.confirm('确定删除选中的角色？', {icon: 3, title: '提示信息'}, function (index) {
                    $.post("${simonBasePath}/role/batchDeleteRole.action?" + ids, function (data) {
                        //刷新table
                        tableIns.reload();
                        //关闭提示框
                        layer.close(index);
        ...
        function openSelectPermisison(data) {
            var index = layui.layer.open({
                ...
                type: 2,
                area: ["400px", "500px"],
                content: "${simonBasePath }/role/toSelectPermission.action?id=" + data.id,
                success: function (layero, index) {
                    setTimeout(function () {
                        layui.layer.tips('点击此处返回角色列表', '.layui-layer-setwin .layui-layer-close', {
                            tips: 3
