...
<script type="text/javascript">
    ...
    layui.use(['form', 'layer', 'table', 'laydate'], function () {
        ...
        tableIns = table.render({
            elem: '#leaveBillList',
            url: '${simonBasePath }/leaveBill/loadAllLeaveBills.action',
            cellMinWidth: 95,
        ...
        $(".search_btn").on("click", function () {
            ...
            table.reload('leaveBillListTable', {
                url: '${simonBasePath }/leaveBill/loadAllLeaveBills.action?' + params
            });
        });
        //监听头工具栏事件
        ...
        function toAddLeaveBill() {
            var index = layui.layer.open({
                ...
                type: 2,//ifream层
                area: ["800px", "500px"],
                content: "${simonBasePath }/leaveBill/toAddLeaveBill.action",
        ...
        function updateLeaveBill(data) {
            var index = layui.layer.open({
                ...
                type: 2,
                area: ["800px", "500px"],
                content: "${simonBasePath }/leaveBill/toUpdateLeaveBill.action?id=" + data.id,
                success: function (layero, index) {
                    setTimeout(function () {
                        layui.layer.tips('点击此处返回请假单列表', '.layui-layer-setwin .layui-layer-close', {
                            tips: 3
        ...
        table.on('tool(leaveBillList)', function (obj) {
            ...
            if (layEvent === 'edit') { //编辑
                ...
                layer.confirm('确定删【' + data.title + '】请假单吗？', {icon: 3, title: '提示信息'}, function (index) {
                    $.post("${simonBasePath}/leaveBill/deleteLeaveBill.action", {
        ...
        function batchDelete() {
            ...
            if (data.length > 0) {
                ...
                layer.confirm('确定删除选中的请假单？', {icon: 3, title: '提示信息'}, function (index) {
                    $.post("${simonBasePath}/leaveBill/batchDeleteLeaveBill.action?" + ids, function (data) {
                        //刷新table
                        tableIns.reload();
                        //关闭提示框
                        layer.close(index);
