...
<script type="text/javascript">
    ...
    layui.use(['form', 'layer', 'laydate', 'table', 'laytpl'], function () {
        ...
        tableIns = table.render({
            elem: '#userList',
            url: '${simonBasePath}/user/loadAllUser.action',
            cellMinWidth: 95,
            page: true,
            height: "full-160",
        ...
        $(".search_btn").on("click", function () {
            ...
            //搜索
            tableIns.reload({
                url: '${simonBasePath}/user/loadAllUser.action?' + params
            })
        });
        ...
        function addUser() {
            var index = layui.layer.open({
                ...
                type: 2,
                area: ["700px", '550px'],
                content: "${simonBasePath}/user/toAddUser.action",
        ...
        function batchDel() {
            ...
            if (data.length > 0) {
                ...
                layer.confirm('确定删除选中的用户？', {icon: 3, title: '提示信息'}, function (index) {
                    $.get("${simonBasePath}/user/deleteUserBatch.action?" + userIds, function (data) {
                        layer.msg(data.msg);
                        tableIns.reload();
                        layer.close(index);
                    })
        ...
        table.on('tool(userList)', function (obj) {
            ...
            if (layEvent === 'edit') { //编辑
                ...
                layer.confirm('确定删除此用户？', {icon: 3, title: '提示信息'}, function (index) {
                    $.post("${simonBasePath}/user/deleteUser.action", {
                        id: data.id
                    }, function (data) {
                        layer.msg(data.msg);
                        tableIns.reload();
                ...
                layer.confirm('确定重置【' + data.name + '】此用户的密码？', {icon: 3, title: '提示信息'}, function (index) {
                    $.post("${simonBasePath}/user/resetPwd.action", {
                        id: data.id
                    }, function (data) {
                        layer.msg(data.msg);
                        layer.close(index);
        ...
        function updateUser(data) {
            var index = layui.layer.open({
                ...
                type: 2,
                area: ["700px", '550px'],
                content: "${simonBasePath}/user/toUpdateUser.action?id=" + data.id,
                success: function (layero, index) {
                    setTimeout(function () {
                        layui.layer.tips('点击此处返回用户列表', '.layui-layer-setwin .layui-layer-close', {
                            tips: 3
        ...
        function openUserRole(data) {
            var index = layui.layer.open({
                ...
                type: 2,
                area: ["700px", '550px'],
                content: "${simonBasePath}/user/toUserSelectRole.action?id=" + data.id,
                success: function (layero, index) {
                    setTimeout(function () {
                        layui.layer.tips('点击此处返回用户列表', '.layui-layer-setwin .layui-layer-close', {
                            tips: 3
