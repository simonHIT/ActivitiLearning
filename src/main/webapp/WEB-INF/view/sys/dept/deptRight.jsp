...
<script type="text/javascript">
    ...
    layui.use(['form', 'layer', 'laydate', 'table', 'laytpl'], function () {
        ...
        tableIns = table.render({
            elem: '#deptList',
            url: '${simonBasePath}/dept/loadAllDept.action',
            cellMinWidth: 95,
            page: true,
            height: "full-160",
        ...
        $(".search_btn").on("click", function () {
            ...
            //搜索
            tableIns.reload({
                url: '${simonBasePath}/dept/loadAllDept.action?' + params
            })
        });
        ...
        function addDept() {
            var index = layui.layer.open({
                ...
                type: 2,
                area: ["700px", '550px'],
                content: "${simonBasePath}/dept/toAddDept.action",
        ...
        function batchDel() {
            ...
            if (data.length > 0) {
                ...
                layer.confirm('确定删除选中的部门？', {icon: 3, title: '提示信息'}, function (index) {
                    $.get("${simonBasePath}/dept/deleteDeptBatch.action?" + deptIds, function (data) {
                        tableIns.reload();
        ...
        table.on('tool(deptList)', function (obj) {
            ...
            if (layEvent === 'edit') { //编辑
                ...
                layer.confirm('确定删除此部门？', {icon: 3, title: '提示信息'}, function (index) {
                    $.post("${simonBasePath}/dept/deleteDept.action", {
                        id: data.id
                    }, function (data) {
                        tableIns.reload();
                        window.parent.left.initTree();//刷新左边的树
        ...
        function updateDept(data) {
            var index = layui.layer.open({
                ...
                type: 2,
                area: ["700px", '550px'],
                content: "${simonBasePath}/dept/toUpdateDept.action?id=" + data.id,
                success: function (layero, index) {
                    setTimeout(function () {
                        layui.layer.tips('点击此处返回部门列表', '.layui-layer-setwin .layui-layer-close', {
                            tips: 3
